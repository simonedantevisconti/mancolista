...
import {
  generateWorldCup2026Cards,
  worldCup2026Series,
} from "../data/worldCup2026Cards";
import { db } from "../firebase";
import { useAuth } from "../hooks/useAuth";
import { exportCollectionPng } from "../utils/exportCollectionPng";
import "../styles/series-detail.css";

const COLLECTION_ID = "mondiali-2026";

const categoryLabels = {
  speciale: "Speciali",
  stemma: "Stemmi",
  giocatore: "Giocatori",
  "foto-squadra": "Foto squadra",
  update: "Update",
};

const WorldCup2026SeriesDetail = () => {
  ...
  const series = worldCup2026Series.find((item) => item.id === seriesId);
  const cards = useMemo(() => generateWorldCup2026Cards(seriesId), [seriesId]);
  ...
  useEffect(() => {
    const loadCards = async () => {
      ...
      try {
        ...
        setError("Non riesco a caricare le figurine salvate.");
      } finally {
        setCardsLoading(false);
      }
    };
  ...
  const toggleCard = async (card) => {
    ...
    try {
      ...
      setError("Non riesco a salvare questa figurina. Riprova.");
    } finally {
      setSavingCardId("");
    }
  };
  ...
  const handleExport = async () => {
    ...
    try {
      await exportCollectionPng({
        collectionName: "FIFA World Cup 2026 Panini",
    ...
    <section className="series-detail">
      <div className="series-topbar">
        <div>
          ...
          <p className="eyebrow">FIFA World Cup 2026 Panini</p>
          <h1>{series.name}</h1>
          <p>
            Segna le figurine che possiedi, controlla le mancanti e indica le
            doppie. Per ora il fronte usa il retro ufficiale caricato nel progetto.
          </p>
          ...
          {cardsLoading && <p className="series-loading">Caricamento figurine...</p>}
        </div>
      ...
      <div className="series-filters">
        <div className="search-field">
          <label htmlFor="world-cup-search">Cerca figurina</label>
          <input
            id="world-cup-search"
            type="search"
            placeholder="Codice, squadra o nome..."
            value={searchTerm}
        ...
        <div className="filter-field">
          <label htmlFor="world-cup-category">Categoria</label>
          <select
            id="world-cup-category"
        ...
        <div className="filter-field">
          <label htmlFor="world-cup-status">Stato</label>
          <select
            id="world-cup-status"
      ...
      {filteredCards.length === 0 && (
        <div className="empty-cards-message">
          <h2>Nessuna figurina trovata</h2>
...
export default WorldCup2026SeriesDetail;
